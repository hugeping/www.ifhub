...
{else}
	...
	<h2 class="page-header">{$aLang.plugin.games.topic_games_edit}</h2>
{/if}
...
<form action="" method="POST" enctype="multipart/form-data" id="form-topic-add" class="wrapper-content">
	...
	{/if}
        ...
        <p>
            <label for="">{$aLang.plugin.games.topic_create_field_link1}:</label>
            <input type="text" name="topic_field_link1" value="{$_aRequest.topic_field_link1}" class="input-text input-width-full" />
            <small class="note">{$aLang.plugin.games.topic_create_description_field_link1}</small>
        </p>

	<input type="hidden" name="topic_type" value="games" />
